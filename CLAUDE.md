...
- **Unresolved targets are opt-in.** By default unresolved links are dropped from `links` entirely. With `settings.includeUnresolved`, they are kept as edges and one placeholder `NoteSnapshot` (`links: []`) is appended per unique unresolved target. A placeholder's `createdAt` is the earliest resolved date among the *linking* notes, inherited verbatim from the winner — derived from the current snapshot each sync and never read back from MongoDB, so `buildSnapshot` stamps no timestamp of its own (ADR-0002). The one clock read left in the pipeline is `resolveCreatedAt`'s `new Date()` last resort for a file with no usable stat; a placeholder inherits it like any other date. Placeholders are ordinary snapshot entries, so the existing `deleteMany` removes them on the first sync after the setting is turned off. A real note always wins a basename — including one outside the subdirectory, which resolves rather than being unresolved. Unresolved link text qualifies only if its basename is extensionless or ends in `.md`; `[[diagram.png]]` and `[[v1.2]]` are dropped.
